/* runtime error capture - prepended for better debugging */
window.addEventListener('error', function(ev){
  try { console.error('Runtime error:', ev.message, ev.error || ev); } catch(e){}
});
...
async function fetchProfileTags() {
  try {
    ...
    const resp = await fetch('http://127.0.0.1:8080/api/profile-tags', {
      method: 'POST',
...
async function fetchLlmScores(recipes) {
  ...
  for (const item of recipes) {
    try {
      const resp = await fetch('http://127.0.0.1:8080/api/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
Evidence summary: ${evidenceAbstract.slice(0, 600)}`
    ...
    let answer = '';
    try{
      const resp = await fetch('http://127.0.0.1:8080/api/deepseek', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
// --- Netlify score & feedback helpers (appended) ---
;(function () {
  async function serverScore(vitals = {}, item = {}, llmScore = null) {
    try {
      const res = await fetch('/api/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ vitals, item, llmScore })
      });
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`score request failed ${res.status}: ${text}`);
      }
      return await res.json();
    } catch (err) {
      console.error('serverScore error', err);
      return { error: String(err) };
    }
  }

  async function submitFeedback(userId, vendor, itemTitle, action) {
    try {
      const res = await fetch('/api/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId, vendor, item_title: itemTitle, action })
      });
      if (!res.ok) {
        const text = await res.text();
  ...
  function renderScoreIntoCard(cardEl, scoreObj) {
    if (!cardEl) return;
    let s = cardEl.querySelector('.server-score');
    let r = cardEl.querySelector('.server-reasons');
    if (!s) {
      s = document.createElement('div');
      s.className = 'server-score';
    ...
    if (!r) {
      ...
      r.className = 'server-reasons';
      r.style.fontSize = '0.9em';
      r.style.color = '#444';
      cardEl.appendChild(r);
    }
    if (scoreObj && scoreObj.error) {
      s.textContent = 'Server error';
      r.textContent = String(scoreObj.error);
      return;
    }
    s.textContent = (scoreObj && typeof scoreObj.score !== 'undefined') ? `Best fit score: ${scoreObj.score}` : '';
    const reasons = (scoreObj && (scoreObj.reasons || scoreObj.reason || scoreObj.explanation)) || [];
    r.textContent = Array.isArray(reasons) ? reasons.join('; ') : String(reasons);
  }

  function attachScoreButtons() {
    document.querySelectorAll('.food-card').forEach(card => {
      if (card.dataset._scoreAttached) return;
      card.dataset._scoreAttached = '1';

      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'score-btn';
      btn.textContent = 'Server Score';
      ...
      btn.addEventListener('click', async (ev) => {
        ...
        // item should be present as JSON in data-item attribute on card
        let item = null;
        if (card.dataset.item) {
          try { item = JSON.parse(card.dataset.item); } catch(e) { item = { title: card.dataset.item }; }
        } else {
          const title = prompt('Enter item title to score (e.g. "Paneer Sprouts Salad")');
          item = { title: title || 'item' };
        }
        // vitals: optional global variable window.currentVitals (if your app sets it)
        const vitals = window.currentVitals || {};
        const result = await serverScore(vitals, item);
        renderScoreIntoCard(card, result);
        btn.disabled = false;
        btn.textContent = 'Server Score';
      });

      // add feedback quick UI (optional) - like/skip buttons if not already present
      const likeBtn = document.createElement('button');
      likeBtn.type = 'button';
      likeBtn.className = 'feedback-like-btn';
      likeBtn.textContent = 'Like';
      likeBtn.style.marginLeft = '6px';
      likeBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const itemTitle = (card.dataset.item && (() => { try { return JSON.parse(card.dataset.item).title } catch(e){ return card.dataset.item } })()) || card.innerText.slice(0,80);
        const resp = await submitFeedback(window.currentUserId || null, card.dataset.vendor || null, itemTitle, 'like');
        // small UX: flash
        likeBtn.textContent = resp && resp.ok ? 'Liked ✓' : 'Like';
        setTimeout(()=> likeBtn.textContent = 'Like', 1200);
      });
      ...
      skipBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const itemTitle = (card.dataset.item && (() => { try { return JSON.parse(card.dataset.item).title } catch(e){ return card.dataset.item } })()) || card.innerText.slice(0,80);
        const resp = await submitFeedback(window.currentUserId || null, card.dataset.vendor || null, itemTitle, 'skip');
        skipBtn.textContent = resp && resp.ok ? 'Skipped ✓' : 'Skip';
        setTimeout(()=> skipBtn.textContent = 'Skip', 1200);
      });

      // place buttons in a control container or at the end of the card
      const controlWrap = document.createElement('div');
      controlWrap.style.marginTop = '8px';
      controlWrap.appendChild(btn);
      controlWrap.appendChild(likeBtn);
      controlWrap.appendChild(skipBtn);

      card.appendChild(controlWrap);
    });
  }

  // auto-attach on DOM ready and expose for later use
  document.addEventListener('DOMContentLoaded', attachScoreButtons);
  window.attachScoreButtons = attachScoreButtons;
  window.serverScore = serverScore;
  window.submitFeedback = submitFeedback;
})();
    
// --- end appended Netlify helpers ---
